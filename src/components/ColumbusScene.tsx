...
export const ColumbusScene: React.FC<{
  readonly src: string;
  ...
  // Smooth Renaissance Nautical Badge spring entrance
  ...
  // Subtle deterministic floating sea-salt / navigator starlight particles
  ...
  return (
    <AbsoluteFill style={{ overflow: "hidden", backgroundColor: "#06101E" }}>
      ...
      {/* Atmospheric Age of Discovery Deep Marine Vignette */}
      <div
        style={{
          position: "absolute",
          inset: 0,
          background:
            "radial-gradient(ellipse at 50% 45%, rgba(0, 0, 0, 0) 38%, rgba(6, 18, 36, 0.45) 70%, rgba(3, 10, 22, 0.94) 100%)",
          pointerEvents: "none",
        }}
      />

      {/* Deep Oceanic Bottom Shade for Subtitle Readability */}
      <div
        style={{
          position: "absolute",
          left: 0,
          ...
          height: 720,
          background:
            "linear-gradient(to top, rgba(3, 9, 20, 0.96) 0%, rgba(5, 14, 28, 0.78) 45%, rgba(7, 20, 40, 0.2) 80%, transparent 100%)",
          pointerEvents: "none",
        }}
      />

      {/* Floating Navigator Sea Salt / Starlight Dust */}
      {particles.map((p, idx) => (
        <div
          key={`particle-${idx}`}
          style={{
        ...
        >
          <div
            style={{
              ...
              boxShadow:
                "0 8px 30px rgba(2, 6, 15, 0.9), inset 0 0 14px rgba(212, 175, 55, 0.22)",
              maxWidth: 960,
            }}
          >
            {/* Badge Text */}
