...
export const VictoriaScene: React.FC<{
  readonly src: string;
  ...
  // Floating Victorian rose petal & stardust particles
  const particles = Array.from({ length: 14 }).map((_, i) => {
    const seed = (i * 89) % 100;
    const xBase = (i * 75 + 45) % 1020;
  ...
  return (
    <AbsoluteFill style={{ overflow: "hidden", backgroundColor: "#08060B" }}>
      ...
      {/* Atmospheric Victorian Mourning Vignette */}
      <div
        style={{
          position: "absolute",
          inset: 0,
          background:
            "radial-gradient(ellipse at 50% 45%, rgba(0, 0, 0, 0) 36%, rgba(14, 8, 20, 0.45) 70%, rgba(6, 3, 9, 0.95) 100%)",
          pointerEvents: "none",
        }}
      />
      ...
      <div
        style={{
          ...
          height: 720,
          background:
            "linear-gradient(to top, rgba(6, 3, 9, 0.96) 0%, rgba(14, 8, 20, 0.8) 45%, rgba(20, 10, 28, 0.2) 80%, transparent 100%)",
          pointerEvents: "none",
        }}
      />

      {/* Floating Rose Gold & Amber Particles */}
      {particles.map((p, idx) => (
        <div
          key={`particle-${idx}`}
          style={{
            ...
            height: p.size,
            borderRadius: "50%",
            backgroundColor: idx % 2 === 0 ? "#FDE68A" : "#FDA4AF",
            opacity: p.opacity,
            boxShadow:
              idx % 2 === 0
                ? "0 0 6px rgba(245, 158, 11, 0.85)"
                : "0 0 6px rgba(225, 29, 72, 0.85)",
            pointerEvents: "none",
          }}
        />
      ))}
        ...
        >
          <div
            style={{
              ...
              boxShadow:
                "0 8px 30px rgba(4, 2, 8, 0.9), inset 0 0 14px rgba(245, 158, 11, 0.22)",
              maxWidth: 960,
            }}
          >
