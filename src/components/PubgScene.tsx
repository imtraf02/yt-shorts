...
export const PubgScene: React.FC<{
  readonly src: string;
  ...
  // Smooth esports scoreboard badge entrance
  ...
  // Tactical embers and digital particles
  const particles = Array.from({ length: 14 }).map((_, i) => {
    ...
    const color = i % 3 === 0 ? "#0EA5E9" : (i % 3 === 1 ? "#F59E0B" : "#EF4444");
    return { x: xBase, y: yPos, opacity, size, color };
  });

  return (
    <AbsoluteFill style={{ overflow: "hidden", backgroundColor: "#0A1218" }}>
      ...
      {/* Atmospheric Tactical Battle Royale Dark Vignette */}
      <div
        style={{
          position: "absolute",
          inset: 0,
          background:
            "radial-gradient(ellipse at 50% 45%, rgba(0, 0, 0, 0) 38%, rgba(10, 18, 24, 0.45) 70%, rgba(5, 10, 14, 0.94) 100%)",
          pointerEvents: "none",
        }}
      />

      {/* Deep Tactical Bottom Shade for Subtitle Readability */}
      <div
        style={{
          position: "absolute",
          left: 0,
          ...
          height: 720,
          background:
            "linear-gradient(to top, rgba(4, 9, 13, 0.96) 0%, rgba(7, 16, 22, 0.78) 45%, rgba(10, 22, 30, 0.2) 80%, transparent 100%)",
          pointerEvents: "none",
        }}
      />

      {/* Floating Tactical Sparks */}
      {particles.map((p, idx) => (
        <div
          key={`particle-${idx}`}
          style={{
        ...
        >
          <div
            style={{
              ...
              boxShadow:
                "0 8px 30px rgba(0, 0, 0, 0.9), inset 0 0 14px rgba(14, 165, 233, 0.25)",
              maxWidth: 960,
            }}
          >
            {/* Badge Text */}
