...
import { PhraseCaption } from "../data/columbusSubtitles";
...
export const ColumbusCaptions: React.FC<{
      ...
      >
        {activePhrase.words.map((w, idx) => {
          ...
          if (isActive) {
            ...
            // Gilded Spanish Doubloon Active Pill
            return (
              <span
                key={`${w.word}-${idx}`}
                style={{
                  ...
                  borderRadius: 14,
                  boxShadow:
                    "0 0 25px rgba(245, 158, 11, 0.85), 0 4px 14px rgba(0,0,0,0.85)",
          ...
          // Inactive Word: Antique Parchment with Deep Oceanic Shadow
          return (
            <span
              key={`${w.word}-${idx}`}
              style={{
                display: "inline-block",
                color: isKeyword ? "#FEF08A" : "#F8F1DE",
                fontSize: 43,
