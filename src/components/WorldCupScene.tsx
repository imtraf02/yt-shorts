...
export const WorldCupScene: React.FC<{
  readonly src: string;
  ...
  // Smooth stadium scoreboard badge entrance
  ...
  // Stadium confetti and floodlight dust particles
  const particles = Array.from({ length: 14 }).map((_, i) => {
    const seed = (i * 97) % 100;
    const xBase = (i * 75 + 40) % 1020;
    const speed = 0.35 + (seed % 5) * 0.12;
    const yPos = (seed * 19 + frame * speed * 2) % 1920;
    const opacity = 0.25 + 0.35 * Math.sin((frame + i * 22) * 0.08);
    const size = 3 + (i % 3) * 1.5;
    const color = i % 3 === 0 ? "#EF4444" : (i % 3 === 1 ? "#22C55E" : "#FBBF24");
    return { x: xBase, y: yPos, opacity, size, color };
  });

  return (
    <AbsoluteFill style={{ overflow: "hidden", backgroundColor: "#08120C" }}>
      ...
      {/* Atmospheric Floodlight Night Stadium Vignette */}
      <div
        style={{
          position: "absolute",
          inset: 0,
          background:
            "radial-gradient(ellipse at 50% 45%, rgba(0, 0, 0, 0) 38%, rgba(8, 20, 12, 0.45) 70%, rgba(4, 12, 7, 0.94) 100%)",
          pointerEvents: "none",
        }}
      />

      {/* Deep Turf Bottom Shade for Subtitle Readability */}
      <div
        style={{
          position: "absolute",
          left: 0,
          ...
          height: 720,
          background:
            "linear-gradient(to top, rgba(3, 10, 6, 0.96) 0%, rgba(5, 16, 9, 0.78) 45%, rgba(7, 22, 12, 0.2) 80%, transparent 100%)",
          pointerEvents: "none",
        }}
      />

      {/* Floating Stadium Confetti Dust */}
      {particles.map((p, idx) => (
        <div
          key={`particle-${idx}`}
          style={{
        ...
        >
          <div
            style={{
              ...
              padding: "9px 24px",
              borderRadius: 9999,
              background: "rgba(9, 23, 14, 0.92)",
              backdropFilter: "blur(14px)",
              border: "1.5px solid rgba(34, 197, 94, 0.65)",
              boxShadow:
                "0 8px 30px rgba(0, 0, 0, 0.9), inset 0 0 14px rgba(34, 197, 94, 0.25)",
              maxWidth: 960,
            }}
          >
            {/* Badge Text */}
