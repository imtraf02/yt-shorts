...
async function main() {
  ...
  const inputPath = path.join(process.cwd(), "temp_16k_elon.wav");

  console.log("Transcribing temp_16k_elon.wav with whisper.cpp...");
  ...
  fs.writeFileSync(
    "whisper_elon_raw.json",
    JSON.stringify(whisperCppOutput, null, 2)
  );

  const { captions } = toCaptions({ whisperCppOutput });
  fs.writeFileSync(
    "whisper_elon_captions.json",
