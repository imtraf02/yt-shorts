...
def main():
    ...
    with open('scripts/worldcup_script.txt', 'r', encoding='utf-8') as f:
        script_text = f.read()

    script_words = script_text.split()
    print(f"Script words: {len(script_words)}")

    with open('whisper_worldcup_captions.json', 'r', encoding='utf-8') as f:
    ...
    # Monotonicity check
    for idx in range(1, len(result_tokens)):
        if result_tokens[idx]['startMs'] < result_tokens[idx-1]['startMs']:
            result_tokens[idx]['startMs'] = result_tokens[idx-1]['endMs']
        if result_tokens[idx]['endMs'] <= result_tokens[idx]['startMs']:
            result_tokens[idx]['endMs'] = result_tokens[idx]['startMs'] + 150

    with open('scripts/aligned_worldcup_tokens.json', 'w', encoding='utf-8') as f:
        json.dump(result_tokens, f, ensure_ascii=False, indent=2)

    print(f"Saved {len(result_tokens)} aligned tokens to scripts/aligned_worldcup_tokens.json")
