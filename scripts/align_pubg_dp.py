...
def main():
    ...
    with open('scripts/pubg_script.txt', 'r', encoding='utf-8') as f:
        script_text = f.read()

    script_words = script_text.split()
    print(f"Script words: {len(script_words)}")

    with open('whisper_pubg_captions.json', 'r', encoding='utf-8') as f:
    ...
    with open('scripts/aligned_pubg_tokens.json', 'w', encoding='utf-8') as f:
        json.dump(result_tokens, f, ensure_ascii=False, indent=2)

    print(f"Saved {len(result_tokens)} aligned tokens to scripts/aligned_pubg_tokens.json")
