...
# Look for foreign words or compound words:
# Incitatus, Caligula, Manche, Neptune, Zeus
foreign_words = ['Incitatus', 'Caligula', 'Manche', 'Neptune', 'Zeus', '24', '1952', '4']
for w in foreign_words:
    print(f"\nOccurrences of '{w}' in script:")
    for idx, sw in enumerate(script_words):
        if w.lower() in sw.lower():
            print(f"  Script[{idx}]: {sw}")

print("\nScanning tokens around Incitatus, Manche, Neptune, Zeus:")
for idx, t in enumerate(tokens):
    txt = t['text'].lower()
    if any(k in txt for k in ['inci', 'tatus', 'man', 'che', 'nep', 'tune', 'zeus', 'dơ', 'dớt']):
        print(f"  Token[{idx}]: '{t['text']}' ({t['startMs']}-{t['endMs']})")
