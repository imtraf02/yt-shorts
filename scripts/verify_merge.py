...
# Merge token 180 ('bơ') and 181 ('nào.') into a single token for 'burnout'
tokens = []
i = 0
while i < len(raw_tokens):
    ...
    if i == 180 and raw_tokens[i]['text'].strip() == 'bơ' and raw_tokens[i+1]['text'].strip() == 'nào.':
        merged = {
            ...
            'confidence': min(raw_tokens[i]['confidence'], raw_tokens[i+1]['confidence'])
        }
...
with open('scripts/comparison_merged.txt', 'w', encoding='utf-8') as out:
...
print("Done writing comparison_merged.txt")
