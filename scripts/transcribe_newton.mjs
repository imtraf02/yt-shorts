...
async function main() {
  ...
  const inputPath = path.join(process.cwd(), "temp_16k_newton.wav");

  console.log("Transcribing temp_16k_newton.wav with whisper.cpp...");
  ...
  fs.writeFileSync(
    "whisper_newton_raw.json",
    JSON.stringify(whisperCppOutput, null, 2)
  );

  const { captions } = toCaptions({ whisperCppOutput });
  fs.writeFileSync(
    "whisper_newton_captions.json",
