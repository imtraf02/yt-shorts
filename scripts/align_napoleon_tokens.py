...
# Now construct the 396 aligned tokens
aligned_tokens = []
w_idx = 0
for t_idx, tok in enumerate(tokens):
    if t_idx == 92: # ' Namps' (22470-22940ms) -> '5' and 'pieds'
        aligned_tokens.append({'text': words[w_idx], 'startMs': tok['startMs'], 'endMs': tok['startMs'] + 230})
        w_idx += 1
        aligned_tokens.append({'text': words[w_idx], 'startMs': tok['startMs'] + 230, 'endMs': tok['endMs']})
        w_idx += 1
    elif t_idx == 233: # ' đã因' (56870-57000ms) -> 'đã' and 'in'
        aligned_tokens.append({'text': words[w_idx], 'startMs': tok['startMs'], 'endMs': tok['startMs'] + 60})
        w_idx += 1
        aligned_tokens.append({'text': words[w_idx], 'startMs': tok['startMs'] + 60, 'endMs': tok['endMs']})
        w_idx += 1
    else:
...
with open('scripts/aligned_napoleon_tokens.json', 'w', encoding='utf-8') as f:
    json.dump(aligned_tokens, f, ensure_ascii=False, indent=2)

print("Saved scripts/aligned_napoleon_tokens.json successfully!")
