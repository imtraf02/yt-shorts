...
def main():
    ...
    with open('scripts/columbus_script.txt', 'r', encoding='utf-8') as f:
        script_text = f.read()

    script_words = script_text.split()
    print(f"Script words: {len(script_words)}")

    with open('whisper_columbus_captions.json', 'r', encoding='utf-8') as f:
    ...
    print(f"Result aligned tokens: {len(result_tokens)} / {len(script_words)} script words")
    
    # Verify strict ascending order
    for idx in range(1, len(result_tokens)):
        prev = result_tokens[idx - 1]
        curr = result_tokens[idx]
        if curr['startMs'] < prev['startMs']:
            curr['startMs'] = prev['startMs']
        if curr['endMs'] <= curr['startMs']:
            curr['endMs'] = curr['startMs'] + 100

    with open('scripts/aligned_columbus_tokens.json', 'w', encoding='utf-8') as f:
        json.dump(result_tokens, f, ensure_ascii=False, indent=2)

    print("Successfully saved to scripts/aligned_columbus_tokens.json")
